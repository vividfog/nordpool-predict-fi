...
# Function to fetch and parse the wind power prediction data from Foreca
# There's a lot GPT-4 code left in this function, but it's mostly about parsing the SVG content
# It's obscure because it has to be, and will hopefully be replaced by a proper API in the future, if available
# The forecast SVG presents the data as bezier curves with a non-linear X-axis and a point-based Y-axis
# The X/Y axis labels are SVG data as well, and their values and X position get updated every few hours
# The function extracts the data, interpolates the missing hours, and converts the data to JSON
# There are not too many hard-coded magic numbers, most variables are derived from the SVG content and its structure/ratios
# The most fragile part is "which of the paths in the SVG is the one we need", as there are more than a 100 paths in the SVG
# Also remember to check how the "color of the vertical line" helps to deduce the non-linear X-axis values, aka grid → hours
# If you run this scrape once a day, the forecast should go far enough into the future to be useful
...
def foreca_wind_power_prediction(wind_power_prediction, data_folder_path):
    ...
    print("Finding the SVG elements")
    ...
    # Close the driver
    print("SVG files have been saved. Closing the web page.")
    driver.quit()
    ...
    # Check if the tree is very small, Foreca has changed the SVG structure, or the file is empty
    if len(root) < 100:  # replace 'threshold' with the minimum number of nodes you expect
        raise ValueError("The SVG tree is too small, Foreca has changed the SVG structure, or the file is empty.")
    ...
    print("Extracting Bezier paths from the SVG")
    ...
    print("Interpolating Y values for each X value")
    for entry in paired_datetime_x:
        ...
        if y_value is not None:  # Only add if y_value could be interpolated
            df_wind_power.append({
                'datetime': entry['datetime'],
                'x': x_value,
                'y': y_value
            })

    # for entry in df_wind_power:
    #     print(f"DateTime: {entry['datetime']}, X: {entry['x']}, Y: {entry['y']}")  # Display the paired datetime, X, and Y values to verify the result
    # print("Total number of entries:", len(df_wind_power))  # Display the length to verify the result

    # # Extract Time (datetime) and Y values from df_wind_power
    # times = [entry['datetime'] for entry in df_wind_power]
    # ys = [entry['y'] for entry in df_wind_power]
    ...
    # Assuming df_wind_power is populated and contains datetime, x, and y values
    # Update the df_wind_power list to include the MW value for each entry
    print("Calculating MW values for each Y entry")
    for entry in df_wind_power:
    ...
    # Extract datetime and MW values for plotting the original Foreca data
    print("Extracting datetime for each MW value for plotting")
    datetimes = [entry['datetime'] for entry in df_wind_power]
    mws = [entry['MW'] for entry in df_wind_power]
    ...
    # Function to interpolate missing hourly entries and their corresponding Y and MW values
    ...
    print("Interpolating the original data to include all hours in the range")
    start_datetime = df_wind_power[0]['datetime']
    end_datetime = df_wind_power[-1]['datetime']
    df_wind_power = interpolate_missing_entries(start_datetime, end_datetime, df_wind_power)

    # Now, df_wind_power contains entries for every hour, including interpolated Y and MW values

    # Define Helsinki timezone
    helsinki_tz = pytz.timezone('Europe/Helsinki')

    # Convert df_wind_power to the required JSON format, adjusting for UTC time
    print("Converting to JSON format and UTC time")
    json_output = []
    for entry in df_wind_power:
