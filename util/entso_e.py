...
TOTAL_CAPACITY = 4372 # TODO: Get from .env.local instead (but this is effectively a constant)
...
def entso_e_nuclear(entso_e_api_key, DEBUG=False):
    """
    Fetches a nuclear production forecast for Finland for the next 5 days.

    This forecast is calculated based on the total known maximum production capacity of Finland's five nuclear plants (OL1, OL2, OL3, Loviisa 1, and Loviisa 2). The forecast adjusts for planned maintenance outages as reported in market messages from ENTSO-E.

    Parameters:
    - entso_e_api_key (str): The API key required for accessing ENTSO-E services.
    - DEBUG (bool, optional): If set to True, enables detailed logging for debugging purposes. Defaults to False.

    Returns:
    - pd.DataFrame: A pandas DataFrame with hourly timestamps for the specified date range, including a column 'NuclearPowerMW' representing the forecasted nuclear production in megawatts.
    - None: If an error occurs during the forecast calculation, the function returns None.
    ...
    try:
        ...
        # Define the start and end times for the forecast period
        ...
        try:
            # Fetch unavailability of generation units for Olkiluoto
        ...
        # Filter for nuclear plant outages at Olkiluoto
        olkiluoto_outages = unavailable_generation[unavailable_generation['plant_type'] == 'Nuclear']
        olkiluoto_outages = olkiluoto_outages[olkiluoto_outages['businesstype'] == 'Planned maintenance']
        ...
        # Fetch unavailability of production units for Loviisa
        ...
        # Filter for nuclear plant outages at Loviisa
        loviisa_outages = unavailable_production[unavailable_production['plant_type'] == 'Nuclear']
        loviisa_outages = loviisa_outages[loviisa_outages['businesstype'] == 'Planned maintenance']
        ...
        # Combine Olkiluoto and Loviisa outages into a single DataFrame
        ...
        # Filter out past outages, keeping only future or ongoing ones
        ...
        # Check for anomalies in outage data
        ...
        # Apply host program naming conventions before returning
