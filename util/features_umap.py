...
def build_feature_embedding(
    ...
    fmisid_ws,
    fmisid_t,
    deploy_folder_path: str,
    output_filename: str = "feature_embedding.json",
) -> str | None:
    if df.empty:
    ...
    frame = pricing.add_time(frame)

    available_t = [col for col in fmisid_t if col in frame.columns]
    available_ws = [col for col in fmisid_ws if col in frame.columns]

    frame = pricing.add_temp(frame, available_t)

    feature_columns = pricing.cols(available_ws, available_t)
    available_features = [col for col in feature_columns if col in frame.columns]
    missing_features = sorted(set(feature_columns) - set(available_features))
    if missing_features:
        logger.info(
            "Feature embedding missing columns skipped: %s", ", ".join(missing_features)
        )
    ...
    zscore = (feature_matrix - feature_matrix.mean(axis=0)) / feature_matrix.std(
        axis=0, ddof=0
    )
    ...
    logger.info(
        "Feature embedding saved to %s (%d features)",
        output_path,
