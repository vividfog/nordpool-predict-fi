...
parser.add_argument(
    "--train",
    action="store_true",
    help="[Deprecated] Train a new model candidate using the data in the database",
)
parser.add_argument(
    "--predict",
    action="store_true",
    help="Train a model (in memory) and display price predictions",
)
parser.add_argument(
    "--narrate",
    action="store_true",
    help="Narrate the predictions into text using an LLM",
)
parser.add_argument(
    "--commit",
    action="store_true",
    help="Commit the predictions/narrations results to DB; use with --predict, --narrate",
)
parser.add_argument(
    "--deploy", action="store_true", help="Deploy the output files to the web folder"
)
parser.add_argument(
    "--dump", action="store_true", help="Dump the SQLite database to CSV format"
)
parser.add_argument(
    "--nn", action="store_true", help="Use neural network model(s) instead of XGBoost"
)
...
if args.train:
    logger.warning(
        "The --train option is deprecated and is no longer used. Training is now performed automatically during --prediction."
    )
...
if args.predict:
    ...
    df_recent = df_recent[
        list(
            set(fmisid_ws + fmisid_t)
            | {
                col
                for col in set(df_recent.columns)
    ...
    df_recent = pricing.add_time(df_recent)
    df_recent = pricing.add_temp(df_recent, fmisid_t)

    # region [predict]
    prediction_features = pricing.cols(fmisid_ws, fmisid_t)
    ...
    df_recent = scale_predicted_prices(
        df_recent, deploy=args.deploy, deploy_folder_path=deploy_folder_path
    )
    ...
    if mask.any():
        ...
        df_recent.loc[mask, "PricePredict_cpkWh"] = df_recent.loc[
            mask, "PricePredict_cpkWh_scaled"
        ]
    else:
    ...
    if drop_cols:
        df_recent = df_recent.drop(columns=drop_cols)
...
if args.commit:
    ...
    if columns_to_drop_existing:
        ...
        df_recent_to_commit = (
            df_recent.copy()
        )  # Or just df_recent if no columns needed dropping

    logger.info(
        f"* Will add/update {len(df_recent_to_commit)} predictions to the database "
    )
    ...
    logger.info(
        "* Predictions generated but NOT committed to the database (no --commit)."
    )
...
if args.deploy and not args.commit:
    logger.warning(
        "--deploy requested without --commit: skipping deploy outputs to avoid stale files."
    )
...
if args.deploy and args.commit:
    ...
    start_of_yesterday_helsinki = datetime.now(helsinki_tz).replace(
    ...
    deploy_df["timestamp"] = (
        deploy_df["timestamp"].dt.tz_localize(None).dt.tz_localize(pytz.utc)
    )
    ...
    hourly_price_predictions["timestamp"] = (
        hourly_price_predictions["timestamp"].dt.tz_localize(None)
        if hourly_price_predictions["timestamp"].dt.tz is not None
        else hourly_price_predictions["timestamp"]
    )
    ...
    create_prediction_snapshot(
        deploy_folder_path, json_data_list, "prediction_snapshot"
    )
    ...
    windpower_preds["timestamp"] = (
        windpower_preds["timestamp"].dt.tz_localize(None)
        if windpower_preds["timestamp"].dt.tz is not None
        else windpower_preds["timestamp"]
    )
    ...
    prediction_full_json = deploy_df.to_json(
    ...
    daily_averages = (
        deploy_df.groupby("timestamp")["PricePredict_cpkWh"].mean().reset_index()
    )
